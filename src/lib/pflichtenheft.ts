...
export const ANFORDERUNGEN: Anforderung[] = [
  ...rows(M.LIE, [
    ...
    ["LIE-009", "Pachtverträge (Fläche, Partner, Laufzeit, Entgelt, Bedingungen, Fristen, Dokumente)", T, "/vertraege", "Pachtvertrag mit Flurstück, Partner (Text), Laufzeit, Betrag, Nutzungsart; PDF jetzt anhängbar. Bedingungen nur in Notizen, Kündigungsfrist Freitext."],
  ...
  ...rows(M.VERTR, [
    ...
    ["VERTR-005", "PDF speichern, öffnen, Objektakte zuordnen", E, "/vertraege", "PDF-Upload im Vertrag (Dialog „Bearbeiten“, nach dem Anlegen), Anzeige im Flurstück-Reiter „Verträge“."],
    ["VERTR-006", "Mehrere Dokumentversionen und Anlagen", T, "/ablage", "Mehrere Anhänge je Vertrag (Vertrag, Nachtrag, Sonstiges). Versionierung nur in der Ablage."],
    ["VERTR-007", "Fristen automatisch in den Kalender", T, "/kalender"],
    ["VERTR-008", "Erinnerungen mit konfigurierbaren Vorlaufzeiten", O],
    ["VERTR-009", "Filter nach Objekt, Partner, Art, Status, Frist", E, "/vertraege", "Filter nach Art, Status, Liegenschaft und Frist (Ende in 30/90/365 Tagen, abgelaufen) sowie Suche nach Bezeichnung und Partner."],
  ]),
  ...rows(M.KAL, [
    ...
    ["KAL-002", "Filter nach Gebäude, Flurstück, Fläche, Anlage, Vertrag, Mitarbeiter, Terminart", T, undefined, "Filter nach Kategorie und Herkunft (Fachmodul). Kein Filter nach Objekt, Fläche oder Mitarbeiter."],
  ...
  ...rows(M.ANL, [
    ["ANL-001", "Vollständige Anlagenliste je Gebäude", T, "/anlagen", "Katalog mit 49 Typen plus Altbestand. Gebäude im Formular wählbar; kein Filter nach Gebäude in der Liste."],
    ["ANL-002", "Räumliche Zuordnung (Gebäude, Etage, Raum, Bereich)", T, "/anlagen", "Gebäude wählbar, sonst Freitext-Standort. Keine Etagen- oder Raumreferenz."],
    ["ANL-003", "Typ, Hersteller, Baujahr, Wartungsfirma, letzte Wartung", E, "/anlagen", "Katalog mit 50 Positionen neu"],
    ["ANL-004", "Wartungsverträge als PDF und im Vertragsmanagement verknüpft", T, "/anlagen", "PDF-Anhang an der Anlage (Typ Vertrag/Wartungsnachweis). Keine Verknüpfung zu Wartungsverträgen im Vertragsmodul."],
    ["ANL-005", "Wartungs-/Prüftermine im Kalender", E, "/kalender"],
    ["ANL-006", "Status, Bemerkungen, Dokumente, Historie", E, "/anlagen", "Status, Notizen, Wartungshistorie und Dokumente (Upload im Anlagenformular)."],
  ]),
  ...rows(M.WART, [
    ["WART-001", "Wartungszyklen und Prüfintervalle je Anlage", E, "/anlagen"],
    ["WART-002", "Letzte/nächste Wartung regelbasiert führen", E, "/instandhaltung", "Einstufung überfällig/≤ 30/≤ 90 Tage; Folgetermin wird bei Wartung/Prüfung aus dem Prüfintervall fortgeschrieben, sofern kein Termin genannt wird."],
    ["WART-003", "Wartungsfirmen und -verträge verknüpfen", T, "/anlagen", "Firma als Text"],
    ["WART-004", "Wartungsnachweise und Protokolle als PDF", T, "/anlagen", "Wartungsnachweise und Prüfberichte als Dokument an der Anlage, nicht am einzelnen Wartungseintrag."],
  ...
  ...rows(M.TKT, [
    ...
    ["TKT-009", "Erledigung, Datum, benötigte Arbeitszeit", E, "/ticketsystem", "Status „Erledigt“ setzt Erledigungsdatum und -person automatisch; Arbeitszeit in Minuten erfassbar; Wiedereröffnen nimmt das Datum zurück."],
    ...
    ["TKT-011", "Historie aller Änderungen", T, "/ticketsystem", "Historie für Status, Zuweisung, Freigabe, Ablehnung sowie Priorität, Fälligkeit, Zuständigen, Kategorie und Kostenstelle. Weitere Felder ohne Eintrag."],
  ...
  ...rows(M.ZAE, [
    ...
    ["ZAE-004", "Zählerart, -nummer, Eigentümer (eigener/Versorger)", T, "/zaehler", "Art und Nummer; Zählernummer je Art eindeutig (409). Eigentümer fehlt."],
  ...
  ...rows(M.DOK, [
    ...
    ["DOK-006", "Volltextsuche in Metadaten; optional OCR", E, "/ablage", "Ablage-Suche über Dateiname, Typ, Metadaten (Schlüssel und Wert) und extrahierten Text; OCR vorhanden."],
  ]),
  ...rows(M.UX, [
    ["UX-001", "Globale Suche über alle Objektarten", T, undefined, "Suche deckt Gebäude, Räume, Wohnungen, Mieter, Liegenschaften, Flurstücke, Verträge, Anlagen, Zähler, Flächen, Tickets, Handwerker und Dokumente ab. Mitarbeiter fehlen."],
    ...
    ["UX-008", "Auswahllisten, Suchfelder, Plausibilitätsprüfungen", E, undefined, "Geprüft werden Fläche ≥ 0, doppelte Lfd. Nr., Ende ≥ Beginn (Vertrag, Reservierung), doppelte Zählernummer, doppeltes Flurstück, rückläufige Zählerstände (Hinweis)."],
  ...
  ...rows(M.TECH, [
    ...
    ["TECH-008", "Sicherung, Wiederherstellung, Protokollierung", T, undefined, "System- und Audit-Log; Sicherungs-/Wiederherstellungsskripte und Konzept liegen vor. readDb() überschreibt defekte Dateien nicht mehr. Kein automatischer Ablauf."],
  ]),
  ...rows(M.SEC, [
    ...
    ["SEC-005", "Backup und Wiederherstellung nach Betriebskonzept", T, undefined, "Sicherungs- und Wiederherstellungsskripte mit Prüfsummen und Konzept in docs/BACKUP_RESTORE.md. Wiederherstellung nur mit Testdaten geprüft, keine Automatisierung."],
  ]),
  ...rows(M.MIG, [
    ...
    ["MIG-004", "Testmigration und Datenqualitätsabnahme", T, undefined, "Testplan mit Abnahmekriterien (docs/MIGRATION_TESTPLAN.md) und Prüfskript check:daten. Noch keine Abnahme mit Produktivdaten."],
